...
class App extends Component {
  ...
  generateRandomImage = () => {
    ...
    this.setState({currentImageId: randomImage.id})
  }
  ...
  render() {
    const {activeTabId, timer, score, currentImageId, isGameOver} = this.state
    const currentImage = imagesList.find(image => image.id === currentImageId)
    return (
          ...
          {!isGameOver ? (
            <>
              ...
              <ul className="images-list-container">
                {this.getFilteredImage().map(eachImage => (
                  <ImageItem
                    imageDetails={eachImage}
                    key={eachImage.id}
                    clickAndChangeImage={this.clickAndChangeImage}
                  />
                ))}
              </ul>
            </>
